...
### #17 — Multi-mineral group members inherited the group's totals + world table
- **Module**: `src/config.py`, `src/pipeline.py`, `src/csv_export.py`
- **Symptom**: Members of true multi-mineral group sheets copied the parent verbatim. `hafnium` and `zirconium` both showed the combined Zr+Hf import total (18,294), the zircon world table, and all 6 import categories (incl. each other's). `silicon-carbide` showed the abrasives group total (261,000) + all 7 abrasive import categories (incl. fused-alumina). Each of the 15 rare-earth aliases showed the group REO mine production/reserves (China 270,000 / 44,000,000) as if they were that element's.
- **Root cause**: Code bug (semantic) — `_make_alias`'s default branch inherited the parent record unchanged for grouped/sub_product aliases without special handling, and the `rare_earth` branch never cleared the inherited `world_production` / import sources.
- **Fix**: "Mechanism B". Each member gets a `parent_filter` keyword + `inherits_world_table` flag (config); `_make_alias` gains a group-member branch + `_fill_group_member` that keeps only the keyword-matching salient rows / import categories and recomputes the summary (zirconium keeps the zircon table, others → world N/A). Members with >1 import form also reuse the Mechanism A per-form split (`_is_per_form_sheet` extended to these aliases), so e.g. zirconium's `compounds` row shows 1,300 not 18,210. The group parents (`zirconium-and-hafnium`, `abrasives`) collapse to a bare sum row (`_category_rows_for`); abrasives fully splits into new `fused-aluminum-oxide` + `metallic-abrasives` aliases so no import shares are lost. `superhard-materials` + the single-mineral downstream products (diamond-powders/gallium-nitride/graphite-anodes/lithium-batteries) blank to N/A (prose kept). The `rare_earth` branch now clears `world_production` + import sources too.
- **Status**: Fixed. Regression tests: `tests/test_csv_export.py::MechanismBGroupTests` (6). Docs: `docs/parsing-special-cases.md` §"Multi-mineral group members". Verified zero change to any primary record.

### #18 — Rare-earth aliases inherited the group Government Stockpile (1,100)
- **Module**: `src/pipeline.py`
- **Symptom**: Found by a full-dataset duplication audit (all 15 REE alias rows were byte-identical apart from name). Every REE alias showed `government_stockpile_fy2025_potential_acquisitions = 1100` — the rare-earths group's lanthanum FY2025 figure — as if it were that element's. A residual leak missed when #17 blanked world + import sources.
- **Root cause**: Code bug — the `rare_earth` branch of `_make_alias` used `_blank_aggregates` (which doesn't touch `stockpile_fy2025_potential_acquisitions`) and never cleared the inherited stockpile (nor the parent's group-level `latest_year_sentinels`).
